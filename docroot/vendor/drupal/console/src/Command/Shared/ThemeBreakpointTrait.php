    ...
    public function breakpointQuestion()
    {
        ...
        while (true) {
            $breakPointName = $this->getIo()->ask(
                $this->trans('commands.generate.theme.questions.max_example-name'),
                'narrow',
            ...
            $breakPointLabel = $this->getIo()->ask(
                $this->trans('commands.generate.theme.questions.max_example-label'),
                $breakPointLabel,
            ...
            $breakPointMediaQuery = $this->getIo()->ask(
                $this->trans('commands.generate.theme.questions.max_example-media-query'),
                'all and (min-width: 560px) and (max-width: 850px)'
            );

            $breakPointWeight = $this->getIo()->ask(
                $this->trans('commands.generate.theme.questions.max_example-weight'),
                '1'
            );

            $breakPointMultipliers = $this->getIo()->ask(
                $this->trans('commands.generate.theme.questions.max_example-multipliers'),
                '1x'
            );
            ...
            if (!$this->getIo()->confirm(
                $this->trans('commands.generate.theme.questions.max_example-add'),
                true
            )
            ) {
                break;
